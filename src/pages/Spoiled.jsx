...
function Spoiled() {
  ...
  // Fetch "Spoiled" tickets (mapped to status='REJECTED' in backend)
  useEffect(() => {
    const fetchSpoiledTickets = async () => {
      try {
        const response = await fetch('/gibco_ticket/api/get-tickets.php?status=REJECTED');
        const data = await response.json();
        setTickets(data || []);
        setCurrentPage(1);
      } catch (error) {
        console.error('Error fetching spoiled tickets:', error);
      }
    };
    fetchSpoiledTickets();
  }, [activeTab]);
  ...
  const filteredTickets = tickets.filter(ticket => {
    ...
    const matchesDate = selectedDate
      ? (ticket.date_rejected || ticket.datetime_rejected || ticket.Log || '').startsWith(selectedDate)
      : true;
    ...
    <main className="flex-1 p-2">
      ...
      <div className="overflow-x-auto bg-white border shadow-sm rounded-xl p-4">
        {paginatedTickets.length === 0 ? (
          <div className="text-center py-10 text-gray-500">No spoiled tickets found.</div>
        ) : (
            ...
            <TableBody>
              {paginatedTickets.map((ticket, index) => (
                <TableRow key={index} className="hover:bg-gray-100">
                  ...
                  <TableCell>{new Date(ticket.datetime_rejected || ticket.date_rejected || '').toLocaleString()}</TableCell>
