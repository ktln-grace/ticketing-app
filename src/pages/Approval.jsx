...
function Approval() {
  ...
  useEffect(() => {
    const fetchApprovalTickets = async () => {
      try {
        const response = await fetch('/gibco_ticket/api/get-tickets.php?status=FOR APPROVAL');
  ...
  const filteredTickets = tickets.filter(ticket => {
    ...
    const matchesDate = selectedDate
      ? (ticket.date_requested || ticket.Log || '').startsWith(selectedDate)
      : true;
    ...
    <main className="flex-1 p-2">
      ...
      <div className="overflow-x-auto bg-white border shadow-sm rounded-xl p-4">
        {paginatedTickets.length === 0 ? (
          <div className="text-center py-10 text-gray-500">No tickets awaiting approval.</div>
        ) : (
            ...
            <TableBody>
              {paginatedTickets.map((ticket, index) => (
                <TableRow key={index} className="hover:bg-gray-100">
                  ...
                  <TableCell>{new Date(ticket.date_requested || ticket.Log || '').toLocaleString()}</TableCell>
                </TableRow>
              ))}
            </TableBody>
          </Table>
